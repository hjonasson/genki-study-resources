...
(function (window, document) {
    ...
    if (storageOK &&
        ...
        /\/privacy\/|\/report\/|\/help\/|\/donate\/|\/download\//.test(window.location.pathname)) {
    ...
    for (var h = document
        ...
        .querySelectorAll("h1, h2, h3, h4, h5, h6"), i = 0, j = h.length; i < j; i++) {
        if (h[i].id) {
            h[i].insertAdjacentHTML("afterbegin", '<a href="#' +
                h[i].id +
                '" class="anchor fa"><span class="anchor-icon">&#xf0c1;</span></a>');
        }
    }
    ...
    if (storageOK) {
        var footer = document.querySelector("footer"), button = document.createElement("LABEL"), container = document.createElement("DIV"), settings = document.createElement("A");
        // set button params
        button.id = "light-switch";
        button.tabIndex = 0;
        button.innerHTML =
            '<input id="light-switch-checkbox" type="checkbox" ' +
                (localStorage.darkMode == "on" ? 'checked="true"' : "") +
                "/><div></div>";
        ...
        button.firstChild.onchange = function () {
            ...
            if (window.KanjiCanvas) {
                KanjiCanvas.darkMode = document.querySelector(".dark-mode")
                    ? true
                    : false;
                KanjiCanvas.quizOver = document.querySelector(".quiz-over")
                    ? true
                    : false;
                for (var a = document.querySelectorAll(".kanji-canvas"), i = 0, j = a.length; i < j; i++) {
                    if (KanjiCanvas["canvas_" + a[i].id])
                        KanjiCanvas.redraw(a[i].id, true, window.Genki ? Genki.strokeNumberDisplay : false);
                }
            }
        };
        ...
        container.innerHTML =
            '<label id="light-switch-label" for="light-switch-checkbox">Dark Mode </label>';
        // settings button
    ...
    // show help link instead of dark mode when running in limited mode
        ...
        var footer = document.querySelector("footer"), container = document.createElement("DIV");
        container.id = "light-switch-container";
        container.innerHTML =
            '<a href="' +
                getPaths() +
                ...
                '"><strong>Where is Dark Mode?</strong></a>';
        footer.appendChild(container);
    }
    // # KEYBOARD SHORTCUTS #
    ...
    document.addEventListener("keydown", function (e) {
        ...
        if (Genki.active.type == "multi" && e.altKey && /a|b|c|d/i.test(e.key)) {
            var opt = document.querySelector("#quiz-q" +
                Genki.stats.solved +
                ' div[data-option="' +
                e.key.toUpperCase() +
                '"]');
