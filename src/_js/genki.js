...
(function (window, document) {
    ...
    var Genki = {
        ...
        feedbackMode: storageOK && localStorage.feedbackMode
            ? localStorage.feedbackMode
            : "classic",
        ...
        lang: {
            ...
            toggle_furigana: '<button id="toggle-furigana" class="button" onclick="Genki.toggle.furigana(this);"><i class="fa">&#xf2a8;</i>' +
                (storageOK && localStorage.furiganaVisible == "false"
                    ? "Show"
                    : "Hide") +
                " Furigana</button>",
        ...
        generateQuiz: async function (o) {
            ...
            *********************************/
            ...
            // # 1. DRAG AND DROP #
            if (o.type == "drag") {
                var quiz = '<div id="quiz-info">' + o.info + '</div><div id="question-list">', dropList = '<div id="drop-list">', keysQ = [], keysA, helper, helperPresent, i;
                ...
                while (keysQ.length) {
                    ...
                    helper = /\|/.test(keysQ[i])
                        ? 'data-helper="' + keysQ[i].split("|").pop() + '"'
                        : null;
                    // add the quiz items and drop zones
                    quiz +=
                        '<div class="quiz-item-group">' +
                            '<div class="quiz-item" ' +
                            (helper || "") +
                            ">" +
                            (helper
                                ? keysQ[i].replace(/(.*?)\|(.*)/, '$1<span class="hidden-text">$2</span>')
                                : keysQ[i]) +
                            "</div>" +
                            "</div>";
                    dropList +=
                        '<div tabindex="0" class="quiz-answer-zone' +
                            (/\|/.test(keysQ[i]) ? " helper-answer" : "") +
                            '" data-text="' +
                            keysQ[i].replace(/\|.*?$/, "") +
                            '" data-mistakes="0"></div>';
                    keysQ.splice(i, 1);
                ...
                while (keysA.length) {
                    ...
                    quiz +=
                        '<div tabindex="0" class="quiz-item" data-answer="' +
                            keysA[i].replace(/\|.*?$/, "") +
                            '"><div class="quiz-item-text">' +
                            o.quizlet[keysA[i]].replace(/\|.*?$/, "") +
                            "</div></div>";
                    keysA.splice(i, 1);
                }
                ...
                zone.innerHTML =
                    quiz +
                        Genki.lang.review.replace("</div>", Genki.lang.toggle_furigana +
                            '<button id="toggle-orientation" class="button" onclick="Genki.toggle.vocabOrientation(this);"><i class="fa" style="position:relative;transform:rotate(90deg);">&#xf0db;</i>Horizontal Mode</button>' +
                            "</div>");
            ...
            // # 2. KANA DRAG AND DROP #
                ...
                var quiz = '<div id="quiz-info">' +
                    o.info +
                    '</div><div id="question-list" class="clear">', answers = '<div id="answer-list">', kanaList = [], kana = o.quizlet, i = kana.length - 1, k;
                    ...
                    for (k in kana[i]) {
                        quiz +=
                            '<div class="quiz-item-row">' +
                                '<div tabindex="0" class="quiz-answer-zone" data-text="' +
                                kana[i][k] +
                                '" data-mistakes="0"></div>' +
                                '<div class="quiz-item">' +
                                kana[i][k] +
                        ...
                        kanaList.push('<div tabindex="0" class="quiz-item" data-answer="' +
                            kana[i][k] +
                            '">' +
                            k +
                            "</div>");
                ...
                zone.innerHTML =
                    quiz + "</div>" + answers + "</div>" + Genki.lang.review;
            }
            // # 3. WRITING PRACTICE #
            ...
            // # 4. MULTIPLE CHOICE #
                ...
                var quiz = '<div id="quiz-info">' +
                    o.info +
                    '<br><b style="color:#6F6;">NEW:</b> You can now choose between "Instant" and "Classic" Feedback Mode for multiple choice quizzes in the <a href="#genki-site-settings" onclick="GenkiSettings.manager(); return false;">Site Settings</a>.' +
                    '</div><div id="question-list">', answers = '<div id="answer-list">', option = 65, // used for tagging answers as A(65), B(66), C(67)..
                isAnswer = false, helper = false, vocab = /"format":"vocab"/.test(Genki.exerciseData), q = o.quizlet, i = 0, j = q.length, n;
                // create individual blocks for each question and hide them until later
                for (; i < j; i++) {
                    quiz +=
                        '<div id="quiz-q' +
                            i +
                            '" class="question-block" data-qid="' +
                            (i + 1) +
                            '" style="display:none;"><div class="quiz-multi-question">' +
                            (o.questionsAlignLeft
                                ? '<div class="quiz-question-inner-text">'
                                : "") +
                            (typeof q[i].question != "undefined"
                                ? q[i].question.replace(/\{.*?\}/g, function (match) {
                                ...
                                : '<div class="text-passage' +
                                    (q[i].vertical ? " vertical-text" : "") +
                                    '" ' +
                                    (q[i].text.replace(/<br>/g, "").length < 50
                                    ...
                                    "</div>" +
                                    (q[i].helper || "")) +
                            (o.questionsAlignLeft ? "</div>" : "") +
                            "</div>" +
                            (vocab
                                ? '<button class="button vocab-spoiler-toggle" onclick="Genki.toggle.vocabSpoiler(this);"><i class="fa"></i>Show Choices</button><div class="vocab-spoiler">'
                                : "");
                    // ready-only questions contain text only, no answers
                    if (q[i].text) {
                        quiz +=
                            '<div class="quiz-multi-row"><div tabindex="0" class="quiz-multi-answer next-question" onclick="Genki.progressQuiz(this, true);" onkeypress="event.key == \'Enter\' && Genki.progressQuiz(this, true);">NEXT</div></div>';
                        ++Genki.stats.exclude; // exclude this block from the overall score
                    }
                    else {
                        // standard question block construction
                        ...
                        while (q[i].answers.length) {
                            ...
                            quiz +=
                                '<div class="quiz-multi-row"><div tabindex="0" class="quiz-multi-answer" data-answer="' +
                                    isAnswer +
                                    '" data-option="' +
                                    String.fromCharCode(option++) +
                                    '" onclick="Genki.progressQuiz(this);" onkeypress="event.key == \'Enter\' && Genki.progressQuiz(this);"><div class="quiz-answer-inner-text">' +
                                    q[i].answers[n] +
                                    "</div></div></div>";
                            isAnswer = false;
                ...
                zone.innerHTML =
                    quiz +
                        "</div>" +
                        '<div id="next-button" class="quiz-multi-row" style="margin-top:-20px; visibility:hidden;' +
                        (Genki.feedbackMode == "classic" ? "display:none;" : "") +
                        '"><div tabindex="0" class="quiz-multi-answer next-question" onclick="Genki.showNextQuestion(this);" onkeypress="event.key == \'Enter\' && Genki.showNextQuestion(this);">NEXT</div></div>' +
                        '<div id="quiz-progress"><div id="quiz-progress-bar"></div></div>' +
                        '<div id="review-exercise" class="center clearfix">' +
                        (Genki.appendix
                            ? ""
                            : '<button class="button text-selection-mode-button" onclick="Genki.toggle.textSelection(this);"><i class="fa">&#xf246;</i> Enable Text Selection</button>') +
            ...
            // # 5. FILL IN THE BLANKS #
                ...
                const hjorturdata = await db.table("helloname").toArray();
                // because of the unorthodox looping via a regexp this is the best way to keep track of the index
                let listIndex = 0;
                zone.innerHTML =
                    '<div id="quiz-info">' +
                        o.info +
                        "<br>If you don't know how to type in Japanese on your computer, please visit our help page by <a href=\"../../../help/writing/" +
                        Genki.local +
                        '" target="_blank">clicking here</a>.</div><div class="text-block">' +
                        o.quizlet.replace(/\{.*?\}/g, function (match) {
                            ...
                            if (data[0] == "!IMG") {
                                ...
                                return ('<div class="audio-block center">' +
                                    ...
                                    getPaths() +
                                    "resources/audio/" +
                                    (Genki.ed == "lessons-3rd" ? "3rd-edition/" : "2nd-edition/") +
                                    data[1] +
                                ...
                                return ('<button class="button play-button" onclick="Genki.playAudio(\'' +
                                    data[1] +
                                    "', " +
                                    data[2] +
                                    ');"><i class="fa">&#xf04b;</i></button>');
                                ...
                                width =
                                    ...
                                    /width:/.test(flag[0])
                                        ? flag[0].split(":")[1]
                                        : flag[1] && /width:/.test(flag[1])
                                            ? flag[1].split(":")[1]
                                            : flag[2] && /width:/.test(flag[2])
                                                ? flag[2].split(":")[1]
                                                : // automatic width calculation between alternate answer params; %(../../../..)
                                                    sub
                                                        ? (hint
                                                            ? ((sub[0] && sub[0][1]) || "/")
                                                                .split("/")
                                                                .concat(((sub[1] && sub[1][1]) || "/").split("/"))
                                                            : ((sub[0] && sub[0][1]) || "/").split("/")).sort(function (a, b) {
                                                            return b.length - a.length;
                                                        })[0].length *
                                                            (14 /
                                                                (/[a-z]/i.test(hint || data[0]) &&
                                                                    !/[\u3000-\u30FF]/.test(hint || data[0])
                                                                    ? 2
                                                                    : 1)) +
                                                            14 +
                                                            ...
                                                            [hint, data[0]].sort(function (a, b) {
                                                                ...
                                                                (14 /
                                                                    (/[a-z]/i.test(hint || data[0]) &&
                                                                        ...
                                                                        : 1)) +
                                                                14;
                                placeholder = /placeholder:/.test(flag[0])
                                    ? flag[0].split(":")[1]
                                    : flag[1] && /placeholder:/.test(flag[1])
                                        ? flag[1].split(":")[1]
                                        : flag[2] && /placeholder:/.test(flag[2])
                                            ? flag[2].split(":")[1]
                                            : null;
                                const previousGuess = hjorturdata.find(({ id }) => id == listIndex);
                                listIndex++;
                                const previousGuessWasCorrect = previousGuess.mistakes === "0";
                                return ('<span class="writing-zone">' +
                                    "<input " +
                                    'class="writing-zone-input" ' +
                                    'type="text" ' +
                                    (previousGuessWasCorrect
                                        ? `value=${previousGuess.answer} `
                                        : " ") +
                                    'data-answer="' +
                                    data[0] +
                                    '" ' +
                                    (flag[0] == "answer" ? 'data-answer2="' + hint + '" ' : "") +
                                    (flag[0] == "answer" && data[3]
                                        ? 'data-answer3="' + data[2] + '" '
                                        : "") +
                                    (flag[0] == "answer" && data[4]
                                        ? 'data-answer4="' + data[3] + '" '
                                        : "") +
                                    (flag[0] == "answer" && data[5]
                                        ? 'data-answer5="' + data[4] + '" '
                                        : "") +
                                    (flag[0] == "answer" && data[6]
                                        ? 'data-answer6="' + data[5] + '" '
                                        : "") +
                                    (flag[0] == "answer" && data[7]
                                        ? 'data-answer7="' + data[6] + '" '
                                        : "") +
                                    (flag[0] == "answer" && data[8]
                                        ? 'data-answer8="' + data[7] + '" '
                                        : "") +
                                    (flag[0] == "answer" && data[9]
                                        ? 'data-answer9="' + data[8] + '" '
                                        : "") +
                                    (flag[0] == "answer" && data[10]
                                        ? 'data-answer10="' + data[9] + '" '
                                        : "") +
                                    (flag[0] == "furigana" ? 'data-furigana="' + hint + '" ' : "") +
                                    (flag[0] == "placeholder" || placeholder
                                    ...
                                    'style="width:' +
                                    width *
                                        (storageOK && localStorage.genkiFontSize
                                            ? +localStorage.genkiFontSize / 100
                                            : 1) +
                                    'px;"' +
                                    ">" +
                                    ((hint && !/answer|furigana|placeholder/.test(flag[0])) ||
                                        (flag[1] && /hint:/.test(flag[1])) ||
                                        (flag[2] && /hint:/.test(flag[2]))
                                        ? '<span class="problem-hint">' +
                                            (flag[1] && /hint:/.test(flag[1])
                                                ? flag[1].split(":")[1]
                                                : flag[2] && /hint:/.test(flag[2])
                                                    ? flag[2].split(":")[1]
                        ...
                        "</div>" +
                        Genki.lang.check_answers
                            .replace("()", "(false, 'fill')")
                            .replace("</div>", helper ? Genki.lang.toggle_furigana + "</div>" : "</div>");
                // auto-focus the first input field
                document.querySelector(".writing-zone-input").autofocus = true;
            }
            // # 6. STROKE ORDER #
            else if (o.type == "stroke") {
                var quiz = '<div id="quiz-info">' + o.info + '</div><div id="question-list">', answers = '<div id="answer-list">', strokeOrderHidden = storageOK && localStorage.strokeOrderVisible == "false", guideHidden = storageOK && localStorage.tracingGuideVisible == "false", q = o.quizlet, i = 0, j = q.length, img;
                // create individual blocks for each question and hide them until later
                for (; i < j; i++) {
                    if (q[i].kana)
                        q[i].kanji = q[i].kana; // applies kana character to kanji property
                    if (o.kanaType)
                        o.kanaType =
                            o.kanaType.charAt(0).toUpperCase() +
                                o.kanaType.slice(1).toLowerCase();
                    img =
                        getPaths() + "resources/images/stroke-order/" + q[i].order + ".png";
                    ...
                    quiz +=
                        '<div id="quiz-q' +
                            ...
                            '<div class="kanji-container big-kanji' +
                            (q[i].kana ? " kana-font" : "") +
                            '"' +
                            (q[i].font ? ' style="font-family:' + q[i].font + '"' : "") +
                            ">" +
                            q[i].kanji +
                            "</div>" +
                            ...
                            (q[i].kana
                                ? getPaths() +
                                    "resources/images/stroke-order/sasagami-" +
                                    o.kanaType.toLowerCase() +
                                    ".jpg"
                                : "https://jisho.org/search/" + q[i].kanji + "%20%23kanji") +
                            '" target="_blank" title="View stroke order ' +
                            (q[i].kana ? "chart" : "on jisho.org") +
                            '"><button class="button"><i class="fa">&#xf002;</i></button></a>' +
                            '<a href="' +
                            img +
                            '" target="_blank" title="Click to view image"><img src="' +
                            img +
                            '" alt="stroke order"/></a>' +
                            "</div>" +
                            "</div>" +
                            ...
                            q[i].strokes +
                            '" data-guide="' +
                            (guideHidden ? false : true) +
                            '"' +
                            (q[i].kana && !q[i].font
                                ? ' data-font="NotoSansJP, SawarabiGothic, MS Gothic, Yu Gothic, Meiryo"'
                                : q[i].font
                                    ? ' data-font="' + q[i].font + '"'
                                    : "") +
                            ' id="canvas-' +
                            ...
                            (q[i].kana ? 'data-kana="true"' : "") +
                            "></canvas>" +
                            "</div>" +
                            '<div class="kanji-canvas-actions quiz-multi-row center">' +
                            '<button class="button" onclick="KanjiCanvas.erase(this.dataset.canvas)" data-canvas="canvas-' +
                            i +
                            '" title="(CTRL+X)"><i class="fa">&#xf12d;</i>Erase</button>' +
                            '<button class="button" onclick="KanjiCanvas.deleteLast(this.dataset.canvas)" data-canvas="canvas-' +
                            i +
                            '" title="(CTRL+Z)"><i class="fa">&#xf0e2;</i>Undo</button>' +
                            (Genki.debug
                                ? '<button class="button" onclick="console.log(KanjiCanvas.recognize(this.dataset.canvas));" data-canvas="canvas-' +
                                    i +
                                    '" title="Open console (F12) to see recognition candidates (CTRL+F)"><i class="fa">&#xf188;</i>Test Recognition</button>'
                                : "") +
                            "</div>" +
                            '<div class="quiz-multi-row">' +
                            '<div tabindex="0" class="quiz-multi-answer next-question" onclick="Genki.progressQuiz(this, false, \'stroke\');" onkeypress="event.key == \'Enter\' && Genki.progressQuiz(this, false, \'stroke\');" data-canvas="canvas-' +
                            i +
                            '">Next ' +
                            (q[i].kana ? o.kanaType : "Kanji") +
                            "</div>" +
                            "</div>" +
                            // end question block
                            "</div>";
                ...
                zone.innerHTML =
                    quiz +
                        '</div><div id="quiz-progress"><div id="quiz-progress-bar"></div></div>' +
                        '<div id="review-exercise" class="center clearfix">' +
                        '<button id="toggle-stroke-order" class="button" onclick="Genki.toggle.strokeOrder(this);"><i class="fa">&#xf1fc;</i>' +
                        (strokeOrderHidden ? "Show" : "Hide") +
                        " Stroke Order</button>" +
                        '<button id="toggle-tracing-guide" class="button" onclick="Genki.toggle.tracingGuide(this);"><i class="fa">&#xf031;</i>' +
                        (guideHidden ? "Show" : "Hide") +
                        " Tracing Guide</button>" +
            ...
            // # 7. DRAWING PRACTICE #
                ...
                var quiz = '<div id="quiz-info">' + o.info + '</div><div id="question-list">', guideHidden = storageOK && localStorage.tracingGuideVisible == "false", columns = o.columns, width = 'style="width:' + 100 / (columns + 1) + '%;"', q = o.quizlet, n = 0, i = 0, j = o.quizlet.length;
                for (; i < j; i++) {
                    if (q[i].kana)
                        q[i].kanji = q[i].kana; // applies kana character to kanji property
                    // create a new row
                    quiz +=
                        '<div class="quiz-answer-row">' +
                            ...
                            "</div>" +
                            "</div>" +
                            '<button class="button stroke-order-button" onclick="Genki.viewStrokeOrder(\'' +
                            q[i].kanji +
                            "', '" +
                            q[i].order +
                            "'" +
                    ...
                    while (columns-- > 0) {
                        quiz +=
                            '<div class="drawing-zone" ' +
                                ...
                                q[i].kanji +
                                '" data-guide="' +
                                (o.columns - columns > 3 ? false : guideHidden ? false : true) +
                                '"' +
                                (q[i].kana && !q[i].font
                                    ? ' data-font="NotoSansJP, SawarabiGothic, MS Gothic, Yu Gothic, Meiryo"'
                                    : q[i].font
                                ...
                                ' data-strokes="' +
                                q[i].strokes +
                                '" data-size="100" id="canvas-' +
                                n +
                                '" width="110" height="110"' +
                                (q[i].kana ? 'data-kana="true"' : "") +
                                "></canvas>" +
                                '<div class="kanji-canvas-actions">' +
                                '<button class="button icon-only" onclick="KanjiCanvas.erase(this.dataset.canvas)" data-canvas="canvas-' +
                                n +
                                '" title="Erase (CTRL+X)"><i class="fa">&#xf12d;</i></button>' +
                                '<button class="button icon-only" onclick="KanjiCanvas.deleteLast(this.dataset.canvas)" data-canvas="canvas-' +
                                n +
                                '" title="Undo (CTRL+Z)"><i class="fa">&#xf0e2;</i></button>' +
                                (Genki.debug
                                    ? '<button class="button icon-only" onclick="console.log(KanjiCanvas.recognize(this.dataset.canvas));" data-canvas="canvas-' +
                                        n +
                                        '" title="Test Recognition (CTRL+F)"><i class="fa">&#xf188;</i></button>'
                                    : "") +
                                "</div>" +
                                "</div>";
                        ++Genki.stats.problems;
                ...
                zone.innerHTML =
                    quiz +
                        "</div>" +
                        Genki.lang.check_answers
                            .replace("()", "(false, 'drawing')")
                            .replace("</div>", '<button id="toggle-tracing-guide" class="button" onclick="Genki.toggle.tracingGuide(this, true);"><i class="fa">&#xf031;</i>' +
                            (guideHidden ? "Show" : "Hide") +
                            " Tracing Guide</button>" +
                            '<button id="toggle-stroke-numbers" class="button" onclick="Genki.toggle.strokeNumbers(this);" style="display:none;"><i class="fa">&#xf162;</i>Show Stroke Numbers</button>' +
                            "</div>");
            ...
            if (Genki.currentAnswers) {
                if (o.type == Genki.currentAnswers.type) {
                    // only apply answers for same type!
            ...
            if (o.type == "drag" || o.type == "kana") {
                ...
                drake.on("drop", function (el, target, source) {
                    ...
                    if (target.dataset.text) {
                        // makes sure the element is a drop zone (data-text == data-answer)
                    ...
                    ["click", "keypress"].forEach(function (eventName) {
                        document.addEventListener(eventName, function (e) {
                            ...
                            while (n-- > 0) {
                                ...
                                if (/quiz-item$|quiz-answer-zone/.test(target.className))
                                    break;
                            }
                            // mark the currently active quiz item
                            if (/quiz-item/.test(target.className) &&
                                target.parentNode.id == "answer-list") {
                                // unmark the last active quiz item
                                if (Genki.markedItem) {
                                    Genki.markedItem.className =
                                        Genki.markedItem.className.replace(" markedItem", "");
                                }
                            ...
                            // attempt dropping the marked quiz item to an answer zone
                                ...
                                /quiz-answer-zone/.test(target.className)) {
                                // wrong answer
                            ...
                            if (e.type == "click" &&
                                /quiz-answer-zone/.test(e.target.className)) {
                                e.target.blur();
                            }
                        });
                    });
            ...
            // exercise timer
            var timer = new Timer(), clock = document.getElementById("quiz-timer");
            ...
            document.getElementById("exercise").className +=
                " content-loaded " +
                    (o.type == "stroke" ? "stroke-quiz multi" : o.type) +
                    "-quiz";
            ...
            if (Genki.debug ||
                (!/drag|kana|drawing/.test(o.type) && !Genki.appendix)) {
                if (Genki.debug ||
                    (o.format && !/vocab|kana|numbers/.test(o.format)) ||
                    !o.format) {
            ...
            if (o.type == "fill" || o.type == "writing") {
                ...
                if (autofocus && autofocus != document.activeElement) {
                    ...
                    Genki.debug &&
                        console.warn("autofocus failed: HTMLElement.focus() will be used as a fallback.");
                }
            }
            // autofocus answer options
            if (o.type == "multi") {
                var q = document.querySelector(document.querySelector(".spoiler-mode")
                    ? ".vocab-spoiler-toggle"
                    : ".quiz-multi-answer");
                if (q)
                    q.focus();
            }
        ...
        incrementProgressBar: function () {
            var bar = document.getElementById("quiz-progress-bar"), progress = Math.floor(((Genki.stats.solved + 1) / Genki.stats.problems) * 100);
            bar.style.width = progress + "%";
            bar.innerHTML =
                '<span id="quiz-progress-text">' +
                    (Genki.stats.solved + 1) +
                    "/" +
                    Genki.stats.problems +
                    "</span>";
        },
        ...
        progressQuiz: function (answer, exclude, flag) {
            ...
            if (answer == "init") {
                ...
                if (answer && flag == "stroke") {
                    ...
                    Genki.debug &&
                        console.log("Correct: " +
                            (new RegExp(answer.dataset.kanji).test(kanji) &&
                                answer.dataset.strokesAnswer == answer.dataset.strokes).toString());
                    // mark the answer as correct or incorrect depending on: 1. the kanji presence and 2. the number of strokes.
                    answer.dataset.answer =
                        new RegExp(answer.dataset.kanji).test(kanji) &&
                            answer.dataset.strokesAnswer == answer.dataset.strokes;
                }
                ...
                var last = document.getElementById("quiz-q" + Genki.stats.solved++), next = document.getElementById("quiz-q" + Genki.stats.solved);
                if (next) {
                    ...
                    // classic progression (answers shown only at end)
                        ...
                        var q = next.querySelector(document.querySelector(".spoiler-mode")
                            ? ".vocab-spoiler-toggle"
                            : ".quiz-multi-answer");
                        if (q)
                            q.focus();
                    ...
                    // end the quiz if there's no new question
            ...
            if (flag == "stroke" &&
                document.getElementById("canvas-" + Genki.stats.solved)) {
        ...
        showNextQuestion: function (caller) {
            ...
            // focus answer for next question
            var q = Genki.multiNodes.next.querySelector(document.querySelector(".spoiler-mode")
                ? ".vocab-spoiler-toggle"
                : ".quiz-multi-answer");
            if (q)
                q.focus();
        ...
        endQuiz: function (type) {
            ...
            var solved = Genki.stats.solved - Genki.stats.exclude, problems = Genki.stats.problems - Genki.stats.exclude;
            ...
            document.getElementById("quiz-result").innerHTML =
                '<div id="complete-banner" class="center">Quiz Complete!</div>' +
                    '<div id="result-list">' +
                    '<div class="result-row"><span class="result-label">Problems Solved:</span>' +
                    problems +
                    "</div>" +
                    '<div class="result-row"><span class="result-label">Answers Wrong:</span>' +
                    Genki.stats.mistakes +
                    "</div>" +
                    '<div class="result-row"><span class="result-label">Score:</span>' +
                    Genki.stats.score +
                    "%</div>" +
                    '<div class="result-row"><span class="result-label">Completion Time:</span>' +
                    timer.innerHTML +
                    "</div>" +
                    '<div class="result-row center">' + // depending on the score, a specific message will show
                    (Genki.stats.score == 100
                        ? "PERFECT! Great Job, you have mastered this quiz! Feel free to move on or challenge yourself by trying to beat your completion time."
                        : Genki.stats.score > 70
                            ? "Nice work! " + Genki.lang[type ? type + "_mistakes" : "mistakes"]
                            : "Keep studying! " +
                                Genki.lang[type ? type + "_mistakes" : "mistakes"]) +
                    (document.querySelector(".alt-phrase")
                        ? "<br><br>" + Genki.lang.sub_answers
                        : "") +
                    '<div class="center">' +
                    (/\/dictionary\//.test(window.location)
                        ? Genki.lang.back_to_dict
                    ...
                    '" class="button"><i class="fa">&#xf015;</i>Back to Index</a>' +
                    "</div>" +
                    "</div>" +
                    "</div>";
            // save results in local storage
            if (Genki.active.exercise.length > 0 &&
                !/appendix|study-tools/.test(Genki.active.exercise[0])) {
                var lesson = Genki.active.exercise[0], genkiEdition = localStorage.GenkiEdition, lessonsResults = JSON.parse(localStorage.Results);
                if (!lessonsResults[genkiEdition])
                    lessonsResults[genkiEdition] = {};
                var editionLessonsResults = lessonsResults[genkiEdition];
                editionLessonsResults[lesson] =
                    typeof editionLessonsResults[lesson] == "undefined" ||
                        Genki.stats.score > editionLessonsResults[lesson]
                        ? Genki.stats.score
                        : editionLessonsResults[lesson];
        ...
        reset: function (skipModal) {
            if (window.JSON) {
                ...
                if (!skipModal &&
                    /fill-quiz|writing-quiz/.test(document.getElementById("exercise").className) &&
                    Genki.stats.mistakes > 0) {
                    return GenkiModal.open({
                        ...
                        callback: function () {
                            ...
                            Genki.currentAnswers = {
                                type: /fill-quiz/.test(document.getElementById("exercise").className)
                                    ? "fill"
                                    : "writing",
                                list: currentAnswers,
                            };
                ...
                var exercise = document.getElementById("exercise"), img = document.querySelector(".multi-quiz-image");
                exercise.className = "content-block";
                exercise.innerHTML =
                    document.getElementById("exercise-title").outerHTML +
                        '<div id="quiz-result"></div><div id="quiz-zone" class="clear"></div>' +
                        (img ? img.outerHTML : "") +
                        '<div id="quiz-timer" class="center"></div>' +
                        document.querySelector(".more-exercises").outerHTML;
                ...
                if (Genki.appendix) {
                    ...
                    Genki.scrollTo(/\/dictionary\//.test(window.location)
                        ? "#practice-words"
                        : ".title");
                    // launch exercise prompt based on the current page
                    if (/\/dictionary\//.test(window.location))
                        Genki.appendix.jisho.launchExercise();
                    else if (/\/map-of-japan\//.test(window.location))
                        Genki.appendix.studyMap();
                    else if (/\/numbers-chart\//.test(window.location))
                        Genki.appendix.studyChart("numbers");
                    else if (/\/conjugation-chart\//.test(window.location))
                        Genki.appendix.studyChart("conjugation");
                }
                // study tools
                else if (Genki.tools) {
        ...
        breakTime: function () {
            GenkiModal.open({
                title: "Take a Break?",
                content: "Taking a break and waiting before trying again can greatly help with building your memory. 5 to 10 minute breaks are recommended, but you're free to adjust the time to your liking." +
                    (/"format":"vocab"|"type":"drag"/.test(Genki.exerciseData)
                        ? ' Please see <a href="' +
                            getPaths() +
                            "help/vocab-memorization/" +
                            Genki.local +
                            '" target="_blank">this page</a> for more tips on memorizing vocab.'
                        : "") +
                    "<br><br>" +
                    '<div class="center">Wait <input id="break-minutes" class="center" type="number" value="' +
                    Genki.breakTimer[Genki.breakMultiplier] +
                    '" min="1" max="60" onchange="Genki.changeBreakMultiplier(this);"> Minute(s)</div>',
                buttonText: "Wait",
                keepOpen: true,
                ...
                callback: function () {
                    ...
                    if (Genki.breakMultiplier < Genki.breakTimer.length - 1) {
                        ...
                        if (Genki.breakMultiplier == 2 && !Genki.breakTimerCustom) {
                            ...
                            if (f &&
                                /helper-present/.test(document.getElementById("quiz-zone").className) == true) {
                                f.click();
                            }
                        }
                    ...
                    if (!Genki.local &&
                        Genki.canNotify &&
                        !/denied|granted/.test(Notification.permission)) {
                    ...
                    GenkiModal.open({
                        ...
                        content: '<div id="break-timer" class="center">00:' +
                            (time < 10 ? "0" : "") +
                            time +
                            ":00</div>",
                        buttonText: "End Break Time",
                        keepOpen: Genki.appendix ||
                            (!Genki.tools && /"format"/.test(Genki.exerciseData))
                            ? true
                            : false,
                    ...
                    document.getElementById("genki-modal-overlay").style.backgroundImage =
                        "url(../../../resources/images/backgrounds/bg-" +
                            (n < 10 ? "0" : "") +
                            n +
                            ".jpg)";
                    // initialize timer
                    var timer = new Timer(), clock = document.getElementById("break-timer");
                    timer.start({
                    ...
                    timer.addEventListener("secondsUpdated", function (e) {
                        ...
                        if (timeString == "00:00:00") {
                            clock.innerHTML =
                                'Break time is up!<div style="font-size:15px;">Click the button below to resume your studies.</div>';
                            document.getElementById("genki-modal-ok").style.display =
                                "inline-block";
                            ...
                            if (!Genki.local &&
                                ...
                                var notif = new Notification(document.title.replace(/ \| Genki Study Resources.*$/, ""), {
                                    ...
                                    icon: document.querySelector('meta[property="og:image"]')
                                        .content,
        ...
        review: function () {
            ...
            GenkiModal.open({
                ...
                callback: function () {
                    var a = document.querySelectorAll("[data-answer]"), i = 0, j = a.length;
                    for (; i < j; i++) {
                        document
                            .querySelector('[data-text="' + a[i].dataset.answer + '"]')
                            .appendChild(a[i]);
                    }
                    ...
                    document.getElementById("review-exercise").innerHTML =
                        (/\/dictionary\//.test(window.location)
                            ...
                            (document.querySelector(".drag-quiz")
                                ? Genki.lang.toggle_furigana +
                                    document.getElementById("toggle-orientation").outerHTML
                                : "");
                    // change the quiz info
                    document.getElementById("quiz-info").innerHTML =
                        'You are currently in review mode; go ahead and take your time to study. When you are ready to practice this exercise, click the "restart" button.';
        ...
        check: {
            ...
            value: function (input) {
                if (!Genki.check.busy && input.value == input.dataset.answer) {
                    ...
                    if (next) {
                        ...
                        window.setTimeout(function () {
                            // delay required since final value seems to be erased when checked immediately
                            input.blur();
                ...
                // checks if currently busy processing the previous answer
                    ...
                    window.setTimeout(function () {
                        // delay required to prevent text duplication when proceeding to already filled inputs
                        Genki.check.busy = false;
                        // use `document.activeElement` over `input` as the latter causes previously input text to disappear on firefox
                        if (document.activeElement &&
                            document.activeElement.value &&
                            document.activeElement.value ==
                                Genki.input.map[Genki.input.index - 1].value) {
                            // clears up duplicated texts from IMEs on current input
                            document.activeElement.value = "";
                        }
                    }, 10);
                }
            ...
            answers: function (mapEnded, type) {
                !Genki.exerciseComplete &&
                    GenkiModal.open({
                        title: "Check Answers?",
                        content: mapEnded
                            ? "The last input field has been filled in. Are you ready to check your answers?"
                            : "Checking your answers will end the quiz. Do you want to continue?",
                        buttonText: "Yes, check my answers!",
                        callback: async function () {
                            Genki.exerciseComplete = true;
                            // hide check answers button
                            document.querySelector("#check-answers button").style.display =
                                "none";
                            ...
                            if (type && type == "drawing") {
                                ...
                                for (; i < j; i++) {
                                    ...
                                    // debugging (logs info about matched kanji, match index, and whether the answer was correct or not)
                                    Genki.debug &&
                                        console.log("toDraw: " + answer[i].dataset.kanji);
                                    Genki.debug && console.log("Results: " + kanji);
                                    Genki.debug &&
                                        console.log("Correct: " +
                                            (new RegExp(answer[i].dataset.kanji).test(kanji) &&
                                                answer[i].dataset.strokesAnswer ==
                                                    answer[i].dataset.strokes).toString());
                                    // correct answer
                                    if (new RegExp(answer[i].dataset.kanji).test(kanji) &&
                                        answer[i].dataset.strokesAnswer == answer[i].dataset.strokes) {
                            ...
                            // standard written quizzes
                                ...
                                var input = document.querySelectorAll("#exercise .writing-zone-input"), i = 0, j = input.length, k, correct, val, data, answer, alt;
                                for (; i < j; i++) {
                                    correct = false;
                                    data = input[i].dataset;
                                    val = Genki.toHalfWidth(input[i].value)
                                        .toLowerCase()
                                        .replace(/。|、|^\s+|\s+$|\n/g, "");
                                    // check for the correct answer
                                    for (k in data) {
                                        if (/answer/.test(k)) {
                                            answer = Genki.toHalfWidth(data[k])
                                                .toLowerCase()
                                                .replace(/。|、|^\s+|\s+$|\n/g, "");
                                    ...
                                    // increment mistakes if the answer is incorrect
                                        ...
                                        ++Genki.stats.mistakes;
                                        if (type == "fill") {
                                            input[i].parentNode.insertAdjacentHTML("beforeend", ('<span class="problem-answer">' +
                                                data.answer +
                                                (data.answer2 || data.furigana
                                                    ? '<span class="secondary-answer' +
                                                        (data.furigana ? " furigana-only" : "") +
                                                        '">' +
                                                        (data.answer2 || data.furigana) +
                                                        "</span>"
                                                    : "") +
                                                "</span>").replace(/%\((.*?)\)/g, function (Match, $1) {
                                                return ('<span class="alt-phrase">(</span>' +
                                                    $1.replace(/\//g, '<span class="alt-phrase-sep">/</span>') +
                                                    '<span class="alt-phrase">)</span>');
                                            }));
                                        }
                                    }
        ...
        toggle: {
            ...
            list: function (el) {
                var closed = "lesson-title", opened = closed + " lesson-open";
            ...
            furigana: function (button) {
                var zone = document.getElementById("quiz-zone"), state = (storageOK && localStorage.furiganaVisible) ||
                    (/helper-hidden/.test(zone.className) ? "false" : "true");
            ...
            vocabOrientation: function (button, customState) {
                var zone = document.getElementById("quiz-zone"), state = typeof customState !== "undefined"
                    ? customState
                    : (storageOK && localStorage.vocabHorizontal) ||
                        (/vocab-horizontal/.test(zone.className) ? "true" : "false"), answer = document.querySelectorAll(".quiz-answer-zone"), i = 0, j = answer.length;
            ...
            strokeNumbers: function (button) {
                ...
                for (var a = document.querySelectorAll(".kanji-canvas"), i = 0, j = a.length; i < j; i++) {
                    if (KanjiCanvas["canvas_" + a[i].id])
                        KanjiCanvas.redraw(a[i].id, false, Genki.strokeNumberDisplay);
                }
            },
            // toggles stroke order in stroke order quizzes
            strokeOrder: function (button) {
                var zone = document.getElementById("quiz-zone"), state = (storageOK && localStorage.strokeOrderVisible) ||
                    (/stroke-order-hidden/.test(zone.className) ? "false" : "true");
                // hide or show the textual aids
                switch (state) {
                    case "true":
                        state = "false";
            ...
            tracingGuide: function (button, drawingPractice) {
                var zone = document.getElementById("quiz-zone"), state = storageOK && localStorage.tracingGuideVisible
                    ? localStorage.tracingGuideVisible
                    : "true";
        ...
        create: {
            ...
            exerciseButtons: function () {
                var more = '<div class="more-exercises clear">', i = 2, a;
                while (i-- > 0) {
                    a =
                        Genki.exercises[i == 1 ? Genki.active.index - 1 : Genki.active.index + 1]; // the prev/next exercise; i=1 is prev, i=0 is next
                ...
                document
                    .getElementById("quiz-timer")
                    .insertAdjacentHTML("afterend", more + "</div>");
            },
            // removes the exercise list when needed update without refreshing the page
            removeExerciseList: function () {
                var list = document.getElementById("exercise-list"), toggle = document.getElementById("toggle-exercises");
                if (list)
            ...
            exerciseList: function () {
                ...
                list = '<nav id="exercise-list">' +
                    ...
                    '<div id="lessons-list"><h4 ' +
                    attrs +
                    '>Page links</h4><ul id="page-links">', lesson = "\\.\\.\\/", i = 0, j = Genki.exercises.length, linkData, active, 
                // vars for grouping sub-sections
                currentGroup, group = "", groups = /workbook-\d+|literacy-\d+|literacy-wb-\d+/, groupTitles = {
                    workbook: "Workbook",
                    literacy: "Reading and Writing",
                    "literacy-wb": "Workbook: Reading and Writing",
                };
                if (storageOK) {
                    localStorage.GenkiEdition = /lessons-3rd/.test(window.location.pathname)
                        ? "3rd"
                        : "2nd";
                    // Create storage for lessons results for specific edition
                    if (!localStorage.Results ||
                        !new RegExp(localStorage.GenkiEdition).test(localStorage.Results)) {
                ...
                for (; i < j; i++) {
                    ...
                    if (!new RegExp(lesson).test(linkData[0])) {
                        lesson = /^appendix/.test(linkData[0])
                            ? "appendix"
                            : /^study-tools/.test(linkData[0])
                                ? "study-tools"
                    ...
                    if (groups.test(linkData[0]) && group != currentGroup) {
                        group = currentGroup;
                        list +=
                            '<li><h4 class="sub-lesson-title">' +
                                groupTitles[group] +
                                "</h4></li>";
                    }
                    // add the exercise link to the group and display results
                    var resultsStorage = JSON.parse(localStorage.Results), editionResultStorage = resultsStorage[localStorage.GenkiEdition], lessonResult = editionResultStorage
                        ? parseInt(editionResultStorage[linkData[0]])
                        : null, resultSpans = {
                    ...
                    }, resultSpan = lessonResult == 100
                        ? resultSpans.perfect
                        : lessonResult >= 70
                            ? resultSpans.good
                            : lessonResult >= 50
                                ? resultSpans.average
                                : resultSpans.low, prevScore = lessonResult > -1
                        ? resultSpan + lessonResult + "%" + "</span>"
                        : "";
                    list +=
                        '<li class="menu-item-list"><a href="' +
                            (lesson == "\\.\\.\\/"
                                ? linkData[0]
                                : "../../../" + Genki.ed + "/" + linkData[0] + "/") +
                            Genki.local +
                            ...
                            (linkData[2]
                                ? 'data-page="Genki ' +
                                    (+linkData[0].replace(/lesson-(\d+).*/, "$1") < 13
                                        ? "I"
                                        : "II") +
                                    (/workbook-|wb-/.test(linkData[0]) ? " Workbook" : "") +
                                    ": " +
                                    linkData[2] +
                                    '"'
                                : "") +
                ...
                document
                    ...
                    .insertAdjacentHTML("afterbegin", '<a href="#toggle-exercises" id="toggle-exercises" onclick="Genki.toggle.exerciseList(this); return false;" title="Toggle exercise list"></a>' +
                    list +
                    "</ul></div></nav>");
                // open the current lesson and scroll to the active exercise
                if (Genki.active.exercise) {
                    // open the active lesson
                    Genki.toggle.list(document.getElementById(/^appendix/.test(Genki.active.exercise[0])
                        ? "appendix"
                        : /^study-tools/.test(Genki.active.exercise[0])
                            ? "study-tools"
                            : Genki.active.exercise[0].replace(/(lesson-\d+)\/.*/, "$1")).previousSibling);
                    ...
                    document.getElementById("lessons-list").scrollTop =
                        active.offsetTop -
            ...
            exerciseTypeButton: function () {
                ...
                var timer = document.getElementById("quiz-timer"), div = document.createElement("DIV"), button = document.createElement("BUTTON");
                ...
                button.onclick = function () {
                    ...
                    GenkiModal.open({
                        ...
                        keepOpen: /\/dictionary\//.test(window.location) ||
                            (!Genki.tools && /"format"/.test(Genki.exerciseData))
                            ? true
                            : false,
        ...
        parse: {
            ...
            image: function (data) {
                ...
                // 1 = file name
                // 2 = alt text (optional)
                var url = /^http/.test(data[1])
                    ? data[1]
                    : "../../../resources/images/lesson-images/" + data[1];
                return ('<a href="' +
                    url +
                    '" target="blank" title="View full image" class="lesson-image"><img src="' +
                    url +
                    '" alt="' +
                    (data[2] || data[1]) +
                    '" /></a>');
        ...
        quickJisho: {
            ...
            create: function () {
                if (Genki.quickJisho.cache)
                    return; // prevent duplication of the quickJisho
                var button = document.createElement("DIV"), box = document.createElement("DIV"), selector = document.createElement("BUTTON"), frag = document.createDocumentFragment();
                // button attrs
                ...
                document.onmousemove = function (e) {
                    Genki.quickJisho.x =
                        Math.abs(e.pageX - document.body.clientWidth) < 100
                            ? e.pageX - 95
                            : e.pageX;
                    Genki.quickJisho.y =
                        Math.abs(e.pageY - document.body.clientHeight) < 40
                            ? e.pageY - 32
                            : e.pageY + 12;
                };
                ...
                document.onkeydown = function (e) {
                    if (e.key == "Tab" &&
                        !Genki.quickJisho.selectorHidden &&
                        document.activeElement != Genki.quickJisho.cache.selector) {
            ...
            search: function (value, retry) {
                ...
                Genki.quickJisho.searchTimeout = window.setTimeout(function () {
                            ...
                            for (i = 0, j = Genki.jisho[k].length; i < j; i++) {
                                for (l in Genki.jisho[k][i]) {
                                    if (Genki.jisho[k][i][l].toLowerCase().indexOf(value) != -1) {
                                        ...
                                        results +=
                                            '<li tabindex="0" class="definition clear">' +
                                                '<span class="def-ja' +
                                                (ja[1] ? " def-furi" : "") +
                                                '">' +
                                                ja[0] +
                    ...
                    // show results
                        ...
                        Genki.quickJisho.cache.results.innerHTML = results
                            ? results
                            : value
                                ? '<li>No results found for "' + value + '".</li>'
                                : "";
                        Genki.quickJisho.cache.hits.innerHTML = hits
                            ? "(" + hits + ")"
                            : "";
                    }
            ...
            lookUp: function () {
                ...
                Genki.quickJisho.cache.search.value = "".trim
                    ? Genki.quickJisho.selectedText.trim()
            ...
            getSelection: function () {
                ...
                if (storageOK && localStorage.genkiJishoLookUp == "false")
                    return false;
                ...
                if (Genki.quickJisho.tabbing) {
                    ...
                    if (!Genki.quickJisho.tabbingOff) {
                        // prevent duplication of timeout
                ...
                if (document.getSelection) {
                    ...
                    if (selection.type == "Range" &&
                        selection.toString &&
                        !/quick-jisho/.test(selection.focusNode.className)) {
                        // stores selected text for searches
                        Genki.quickJisho.selectedText = selection.toString();
                        // update lookup button position
                        Genki.quickJisho.cache.selector.style.left =
                            Genki.quickJisho.x + "px";
                        Genki.quickJisho.cache.selector.style.top =
                            Genki.quickJisho.y + "px";
                        // show lookup button
                        ...
                        // hide lookup button and clear selection
                        Genki.quickJisho.selectedText = "";
                        if (!Genki.quickJisho.selectorHidden) {
        ...
        viewStrokeOrder: function (kanji, order, kana) {
            ...
            GenkiModal.open({
                ...
                content: '<div class="kanji-stroke-order center">' +
                    '<div class="big-kanji' +
                    (kana ? " kana-font" : "") +
                    '"' +
                    (/り/.test(kanji)
                        ? ' style="font-family:SawarabiGothic, MS Gothic, Yu Gothic, NotoSansJP, Meiryo;"'
                        : "") +
                    ">" +
                    kanji +
                    "</div>" +
        ...
        getAlts: function (str, alt, arrayOnly) {
            var subs = alt.split("|"), length = subs.length, permutations = Math.pow(2, length), results = [], i = 0, bit, bitIndex, subIndex, result;
            ...
            if (Genki.debug) {
                ...
                console[(len / permutations) * 100 == 100 ? "log" : "warn"](len +
                    "/" +
                    permutations +
                    " (" +
                    (len / permutations) * 100 +
                    "% combo coverage for " +
                    length +
                    " replacements; " +
                    (permutations - len) +
                    " missing combos)", results);
            }
        ...
        randomExercise: function () {
            ...
            if (storageOK &&
                localStorage.genkiRandomExercise == "lesson" &&
                /lesson-\d+/.test(window.location.href)) {
                var regex = new RegExp(window.location.href.replace(/.*?(lesson-\d+).*/, "$1/")), list = Genki.exercises.filter(function (a) {
                    return regex.test(a);
                });
            }
            // random exercise preference (random, previously completed lesson)
            else if (storageOK &&
                localStorage.genkiRandomExercise == "completed" &&
                localStorage.Results &&
                JSON.parse(localStorage.Results)[localStorage.GenkiEdition]) {
                var editionLessonResults = JSON.parse(localStorage.Results)[localStorage.GenkiEdition];
                var list = Genki.exercises.filter(function (a) {
                    return a.split("|")[0] in editionLessonResults;
                });
                if (!list[0]) {
        ...
        startOrPauseTimerByVisibility: function () {
            if (document.hidden && Genki.timer.isRunning()) {
                ...
                Genki.timer.isPaused() &&
                !Genki.isTimerPausedByPopup) {
                Genki.timer.start();
            }
        },
        ...
        init: function () {
            ...
            var i = 0, j = Genki.exercises.length, result = document.getElementById("quiz-result"), lesson;
            for (; i < j; i++) {
                if (Genki.active.path == Genki.exercises[i].split("|")[0]) {
                    Genki.active.exercise = Genki.exercises[i]
                        ? Genki.exercises[i].split("|")
                        : null;
            ...
            if (Genki.active.exercise) {
                lesson = /^appendix/.test(Genki.active.exercise[0])
                    ? "appendix"
                    : /^study-tools/.test(Genki.active.exercise[0])
                        ? "study-tools"
                        : +Genki.active.exercise[0].replace(/lesson-(\d+).*/, "$1"); // current lesson
                result.insertAdjacentHTML("beforebegin", '<h2 id="exercise-title" class="center" ' +
                    (Genki.active.exercise[2]
                        ? 'data-page="Genki ' +
                            (lesson < 13 ? "I" : "II") +
                            (/workbook-|wb-/.test(Genki.active.exercise[0])
                                ? " Workbook"
                                : "") +
                            ": " +
                            Genki.active.exercise[2] +
                ...
                result.insertAdjacentHTML("beforebegin", '<h2 id="exercise-title" class="center">' +
                    document.querySelector("TITLE").innerText.replace(/\s\|.*/, "") +
                    "</h2>");
            }
            ...
            if (Genki.isTouch) {
                ...
                document.addEventListener("touchmove", function (e) {
                    if (Genki.isTouching &&
                        /hidden/i.test(document.body.style.overflow)) {
                        e.preventDefault();
                    }
                }, { passive: false });
    ...
    window.onbeforeunload = function () {
        var lossDetected = false, type = document.getElementById("exercise").className;
        // determine exercise type and find if the user may incur progress loss for the current exercise
        if (/quiz-over/.test(type) || document.querySelector(".review-mode")) {
            // ignore this check completely if the quiz is over or student is in review mode
            lossDetected = false;
        }
        // check if any of the inputs have been filled in for a written quiz
            ...
            for (var a = document.querySelectorAll(".writing-zone-input"), i = 0, j = a.length; i < j; i++) {
                if (a[i].value != "") {
                    // mark as a "loss" if an input is filled in and break out of the loop
                    lossDetected = true;
                    break;
                }
            }
        ...
        // check if any of the canvases have been drawn on
            ...
            for (var a = document.querySelectorAll(".kanji-canvas"), i = 0, j = a.length; i < j; i++) {
                if (KanjiCanvas["recordedPattern_" + a[i].id].length) {
                    // mark as a "loss" if a canvas has been drawn on and break out of the loop
                    lossDetected = true;
                    break;
                }
            }
        ...
        else if (/multi-quiz|drag-quiz|kana-quiz/.test(type) &&
            Genki.stats.solved > 0) {
            lossDetected = true;
        }
...
function oIsObject(o) {
    ...
    var begin = /(?:begin|start)=\d/.test(window.location.search)
        ? window.location.search.replace(/.*?(?:begin|start)=(\d).*/, "$1")
        : false, i = 0, j = o.type.length, opts = "", modal;
    // parse options
    for (; i < j; i++) {
        opts +=
            '<option value="' +
                ...
                (begin !== false
                    ? i == begin
                        ? " selected"
                        : ""
                    : storageOK && localStorage["genki_pref_" + o.format] == o.type[i]
                        ? " selected"
                        : "") +
                ">" +
                Genki.lang.opts[o.format][o.type[i]] +
                "</option>";
    }
    ...
    modal = GenkiModal.open({
        ...
        content: 'Please select the type of exercise you would like to do, then click "Begin" to start studying.<br><br>' +
            '<div class="center">' +
            (/\/dictionary\/|\/custom-vocab\//.test(window.location)
                ? ""
                : "<div>" +
            ...
            '<input id="modal-skip-ex-type" class="genki_input_hidden" type="checkbox"' +
            (storageOK && localStorage.genkiSkipExType == "true" ? " checked" : "") +
            ' onchange="localStorage.genkiSkipExType = this.checked">' +
        ...
        callback: function () {
            ...
            o.type = o.type[type];
            o.info =
                typeof o.info == "object" && o.info[type] ? o.info[type] : o.info;
            ...
            if (/vocab|kana|numbers|kanji/.test(o.format)) {
                ...
                if (o.format == "kanji") {
                    ...
                    if (o.readings) {
                        ...
                        for (i in o.quizlet) {
                            ...
                            o.quizlet[i] =
                                (yomikata[0]
                                    ? '<span class="yomikata" title="on-yomi">▶</span>' +
                                        yomikata[0] +
                                        "<br>"
                                    : "") +
                                    (yomikata[1]
                                        ? '<span class="yomikata" title="kun-yomi">▷</span>' +
                                            yomikata[1]
                                        : "");
                        }
                    ...
                    if (o.type == "drag") {
                        document.getElementById("exercise").className += o.readings
                            ? " kanji-readings"
                            : o.meanings
                                ? " kanji-meanings"
                                : "";
                ...
                if (o.type == "multi") {
                    ...
                    for (i = 0, j = keys.length; i < j; i++) {
                        ...
                        sentence = /\|/.test(o.quizlet[keys[n]])
                            ? o.quizlet[keys[n]].replace(/.*?\|(.*?$)/, "$1")
                            : "";
                            ...
                            question: '<div class="multi-vocab">' +
                                ...
                                (def[1]
                                    ...
                                    : o.format == "kanji"
                                        ...
                                        : "") +
                                (sentence
                                    ? '<hr><div class="multi-vocab-sentence">' +
                                        sentence +
                                        "</div>"
                                    : "") +
                                "</div>",
                ...
                // write the definition conversion
                    ...
                    if (/vocab|numbers/.test(o.format)) {
                        var quizlet = '<div class="count-problems columns-2 clear"><div>', keys = [], def, problem, i, j, n;
                        // get keys
                        for (i in o.quizlet) {
                            keys.push(i);
                        }
            ...
            // switches between the workbook format and stroke order format
                ...
                o.quizlet = o.quizlet[type]
                    ? o.quizlet[type]
                    : o.quizlet[o.quizlet.length - 1];
            }
    ...
    if (!Genki.changingExType &&
        (begin !== false || (storageOK && localStorage.genkiSkipExType == "true"))) {
        modal.callback();
        GenkiModal.close();
    }
...
function oIsWriting(o, zone) {
    // get data from database and apply in the quiz += line
    var quiz = '<div id="quiz-info">' +
        o.info +
        "<br>If you don't know how to type in Japanese on your computer, please visit our help page by <a href=\"../../../help/writing/" +
        Genki.local +
        '" target="_blank">clicking here</a>.</div><div id="question-list">', columns = o.columns, width = 'style="width:' + 100 / (columns + 1) + '%;"', index = 0, helper = false, i, j;
    for (i in o.quizlet) {
        // create a new row
        quiz +=
            '<div class="quiz-answer-row' +
                (o.quizlet[i] ? " furi-row" : "") +
                '"><div class="quiz-item" data-helper="' +
                o.quizlet[i] +
                '" ' +
                width +
                '><div class="quiz-item-text">' +
        ...
        while (columns-- > 0) {
            quiz +=
                '<div class="writing-zone" ' +
                    width +
                    '><input class="writing-zone-input" type="text" ' +
                    (!o.quiz && j++ < 3 ? 'placeholder="' + i + '"' : "") +
                    ' data-answer="' +
                    (o.quiz ? o.quizlet[i] : i) +
                    '" data-mistakes="0" tabindex="0" ' +
                    (o.quiz ? "" : 'oninput="Genki.check.value(this);"') +
                    ' onfocus="Genki.input.index = ' +
                    index++ +
                    '"></div>';
            ++Genki.stats.problems;
        }
    ...
    zone.innerHTML =
        quiz +
            "</div>" +
            Genki.lang.check_answers.replace("</div>", helper ? Genki.lang.toggle_furigana + "</div>" : "</div>");
